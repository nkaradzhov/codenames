...
import { gameRooms, gameRoomAdded, gameRoomPlayers, apiError } from '../actions/socketapi'
...
class SocketApi extends React.Component {
  constructor(args) {
    ...
    this.channel.on('players', players => this.props.dispatch(gameRoomPlayers(players)))
    this.channel.on('warn', error => {
      console.log('???');
